...
###example3:     ABOUT TSLA      ==============================================
getSymbols("TSLA", from = "2018-01-01", to = "2020-10-27")
TSLA.rtn = diff(log(TSLA$TSLA.Adjusted))
TSLA.price = TSLA$TSLA.Adjusted
chartSeries(TSLA)
chartSeries(TSLA.rtn,theme = "black", name = "The return of TSLA from 2018-01-03 to 2020-10-26")
chartSeries(TSLA.price,theme = "white", name = "The price of TSLA from 2018-01-02 to 2020-10-26")
basicStats(TSLA.price)#compute summary statistics about the price of TSLA
basicStats(TSLA.rtn)#compute summary statistics about the return of TSLA


###part2:distribution of financial data;plot the histogram, the fitted density and normal density in one figure
TSLA.rtn1 = na.omit(TSLA.rtn)
hist(TSLA.rtn1, nclass = 40, freq = F, main = "Histogram of TSLA's return", xlab = "TSLA_return")
d3 = density(TSLA.rtn1)
range(TSLA.rtn1)
x3 = seq(-0.4,0.4,0.001)
y3 = dnorm(x3, mean(TSLA.rtn1),stdev(TSLA.rtn1))
lines(d3$x, d3$y, xlab = "x", ylab = "density", lty = 1, col = "blue")
lines(x3, y3, lty = 2, col = "red")


### part3: normality test
t.test(TSLA.rtn1)  # Testing mean return = 0
s3=skewness(TSLA.rtn1)
T=length(TSLA.rtn1) # Sample size
t3=s3/sqrt(6/T) # Skewness test
pp=2*(1-pnorm(t3)) # Compute p-value
s4=kurtosis(TSLA.rtn1)
t4=s4/sqrt(24/T) # Kurtosis test
normalTest(TSLA.rtn1,method='jb') # JB-test



### part4:Save data
write.csv(TSLA, file = "TSLA.csv")
write.csv(TSLA.rtn,file = "TSLA_rtn.csv")
